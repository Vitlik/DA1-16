...
d.b.outlier_detection <- function(){
  ...
  # The function "lofactor" identifies outliers by calculating the multivariate distances. It returns a
  # vector of local outlier factors of each object in the data.
  # The parameter "neigbour" (k) is set to 100, since there are a large number duplicated raws in the dataset. In
  # oder to avoid generating NAs, there should be more neigbours to be compared.
  lof.det<-DMwR::lofactor(spambase.scaled,k=100)
  # plot the outliers and their IDs in the dataset. Those objects, whose values are greater than the mean
